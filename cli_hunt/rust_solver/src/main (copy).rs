...
fn main() {
    ...
    let start_nonce = 0;
    let nonce_max = args.nonce_max; // Lấy giá trị từ args
    ...
    if found.load(Ordering::Relaxed) {
        ...
        eprintln!("Error: Solver failed (nonce_max {} exceeded or no solution found).", nonce_max);
        std::process::exit(1);
    }
    // --- KẾT THÚC SỬA ĐỔI ---
}
